...
function RunTerminator()
	...
	shell.run "C:\Windows\System32\wsl.exe zsh -c ""cd " & cdPath & "; JAVA_HOME=/home/samhv/.sdkman/candidates/java/current DISPLAY=$(netstat -rn | grep -o ""^0.0.0.0 \+\([0-9]\.\?\)\+"" | grep -o ""[^ ]\+$""):0 $HOME/idea/idea-IC-203.8084.24/bin/idea.sh""", 0, true
end function

function KillXserverProcess ( strProcess )
	'Check if another bash process is running to avoid closing xServer
	if Not getProcessObject("zsh") is Nothing Then
		exit function
	end if
