...
3. **Render** — `(cd "$PROJECT_DIR" && npx hyperframes render . -q <quality> -o ./renders/video.mp4)` (add `--format webm` for an alpha overlay export). Verify the mp4 exists + duration matches.
