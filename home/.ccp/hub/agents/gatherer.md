---
name: gatherer
description: "Gather worker agent — spawned by lead-researcher skill with assigned language(s) and topic. Do NOT spawn this agent directly — always go through lead-researcher first, which decides depth, assigns languages, and handles synthesis. Each gatherer agent focuses on its assigned language/topic and runs the search-fetch loop from the deep-gather skill."
---
...
Your caller specified: **topic**, **language(s)**, and **output path**. Follow those exactly.
- Search in your assigned language(s) — don't default to English unless assigned English
- Loop search + fetch + evaluate until new searches stop surfacing new sources
- Stay within your topic boundaries — don't overlap with sibling agents

## Methodology

Load the `deep-gather` skill before searching — it holds the search-fetch loop, query templates, GitHub patterns, and retrieval tools this job depends on.
