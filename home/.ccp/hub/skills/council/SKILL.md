---
name: council
description: "Live collaborative brainstorm: named teammates (agent teams) build on each other's ideas via SendMessage ('yes, and…'), then you synthesize. Use when the user types /council or wants an open question ('how might we…', 'what could we build') explored by perspectives that compound each other live — brainstorm together, ideate as a team, expand on this. Requires the main session and CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1. Not for a quick idea list (solo agents), choosing between options (/debate), or mechanical tasks."
---
